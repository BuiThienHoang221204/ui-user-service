...
import { ArrowLeft, UserCheck, AlertCircle, ShieldAlert } from 'lucide-react';
...
export const EditUserPage: React.FC = () => {
  ...
  // Queries current details
  const { data: user, isLoading: isUserLoading, error: userError } = useUserDetail(id || '');
  ...
  // Sync loaded user to form states
  useEffect(() => {
    if (user) {
      setFormData({
  ...
  const updateMutation = useUpdateUser(() => {
    setToast({
      message: `Đã cập nhật thành công thông tin của ${formData.fullName}!`,
      type: 'success'
    });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    // Clear error on type
    if (errors[name as keyof UserUpdateInput]) {
      setErrors(prev => ({
        ...prev,
        [name]: undefined,
      }));
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateMutation.mutate({
      ...
      onError: (err: any) => {
        ...
        const msg = err?.response?.data?.message || err?.message || 'Có lỗi xảy ra khi cập nhật tài khoản!';
        if (status === 409) {
          setToast({
            ...
            <div className="px-8 py-6 border-b border-slate-800/40 flex items-center gap-3.5 bg-slate-950/20">
              ...
              <div className="flex flex-col gap-0.5">
                ...
                <span className="text-[10px] text-slate-500 font-semibold">
                  Cập nhật hồ sơ và tùy chỉnh vai trò của thành viên ID #{user.id}.
                </span>
              </div>
            </div>
